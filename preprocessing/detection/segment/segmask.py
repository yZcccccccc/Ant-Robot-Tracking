        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if save_masks:
                    ...
                    if len(det) == 0:
                        black_mask = np.zeros((im0.shape[0], im0.shape[1]), dtype=np.uint8)
                        mask_save_path = str(save_dir / 'mask' / f'{p.stem}.png')
                        cv2.imwrite(mask_save_path, black_mask)

                    else:
                        # Create an empty mask with the same size as the image
                        merge_mask = np.zeros((im0.shape[0], im0.shape[1]), dtype=np.uint8)

                        # Merge all masks into the merge_mask
                        for mask in masks:
                            binary_mask = (mask > 0.5).to(dtype=torch.uint8) * 255
                            binary_mask = binary_mask.cpu().numpy()
                            merge_mask = np.maximum(merge_mask, binary_mask)

                        # Save the merged mask as an image
                        mask_save_path = str(save_dir / 'mask' / f'{p.stem}.png')
                        cv2.imwrite(mask_save_path, merge_mask)




                        # mask_save_path = str(save_dir / f'{p.stem}_mask.jpg')
                        # cv2.imwrite(mask_save_path, im_masks)
...
def parse_opt():
    ...
    parser.add_argument('--save-masks', action='store_true', help='save masks to image files')
